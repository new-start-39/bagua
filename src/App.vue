...
onBeforeUnmount(() => globalThis.removeEventListener('bagua:auth-required', handleAuthRequired))
</script>
...
.history-drawer { position: absolute; top: 0; right: 0; display: flex; width: min(430px,92vw); height: 100%; flex-direction: column; padding: 34px clamp(22px,4vw,42px); border-left: 1px solid rgba(92,245,212,.24); background: linear-gradient(160deg,rgba(13,21,55,.98),rgba(18,11,35,.98)); box-shadow: -18px 0 70px rgba(0,0,0,.34); }
.history-drawer-heading { display: flex; justify-content: space-between; padding-bottom: 25px; border-bottom: 1px solid rgba(121,144,215,.18); h2 { margin: 10px 0 0; color: #eef2ff; font-size: 31px; } }
...
.history-list { display: grid; flex-shrink: 1; gap: 8px; overflow-y: auto; margin: 12px 0 0; padding: 0; list-style: none; }
.history-list-local { flex-shrink: 0; max-height: 32vh; margin-bottom: 20px; }
...
.history-clear { align-self: flex-start; border: 0; background: transparent; color: #c886a9; }
.history-more { align-self: center; min-height: 40px; border: 1px solid rgba(92,245,212,.35); padding: 0 14px; background: transparent; color: #8ff3e3; }
.history-note { margin-top: auto; color: #58658f; font: 10px 'DM Mono',monospace; }
...
@media (max-width: 760px) { .app-shell { padding: 22px 18px 32px; } .app-shell.chat-shell { display: block; height: auto; min-height: 100vh; min-height: 100dvh; overflow: visible; padding-bottom: 18px; } .system-status, .motion-toggle span, .history-button span { display: none; } .topbar-tools { gap: 7px; } }
</style>
