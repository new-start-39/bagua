...
const initialize = async () => {
  ...
  try {
    ...
    if (generation === initializeGeneration) loadError.value = caught.message
  } finally {
...
<template>
  <section class="chat-page" aria-labelledby="chat-title">
    ...
    <div v-else-if="loadError" class="center-state error"><span>CONTEXT UNAVAILABLE</span><h1 id="chat-title">无法开始解读</h1><p>{{ loadError }}</p><button type="button" @click="returnToDivination">返回卦象</button></div>
    <template v-else>
